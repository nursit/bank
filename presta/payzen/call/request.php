...
if (!defined('_PAYZEN_VADS_VERSION')){
	include_spip('presta/payzen/config');
}
...
function presta_payzen_call_request_dist($id_transaction, $transaction_hash, $config = array(), $action = "PAYMENT", $options = array()){
	...
	$cartes_possibles = payzen_available_cards($c);
	...
	$parm['vads_version'] = _PAYZEN_VADS_VERSION;
	...
	if (isset($cartes_possibles['SDD'])
		...
		$contexte = presta_payzen_call_request_dist($id_transaction, $transaction_hash, $config_sdd, $action_sdd, $options_sdd);
		unset($cartes_possibles['SDD']);
	} else {
		$contexte = array(
			'hidden' => array(),
			'action' => payzen_url_serveur($config),
	...
	foreach ($cartes as $carte){
		if (isset($cartes_possibles[$carte])){
			...
			$contexte['hidden'][$carte] = payzen_form_hidden($config, $parm);
