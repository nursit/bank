...
function bank_inserer_transaction_dist($montant, $options = array()){
	...
	if (count($args)>2 OR (isset($args[1]) AND !is_array($args[1]))){
		$options = array();
		if (isset($args[7]) AND is_array($args[7])){
			$options['champs'] = $args[7];
		}
		...
		if (isset($args[1])){
			$options['montant_ht'] = $args[1];
		}
		if (isset($args[2])){
			$options['id_auteur'] = $args[2];
		}
		if (isset($args[3])){
			$options['auteur_id'] = $args[3];
		}
		if (isset($args[4])){
			$options['auteur'] = $args[4];
		}
		if (isset($args[5])){
			$options['parrain'] = $args[5];
		}
		if (isset($args[6])){
			$options['tracking_id'] = $args[6];
		}
	}
